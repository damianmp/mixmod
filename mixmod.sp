...
new Handle:g_RoundStartForward = INVALID_HANDLE;
new Handle:g_RoundEndForward = INVALID_HANDLE;
...
new bool:ENABLE_PLUGIN_CHECKING = true;
...
public Native_IsMixmodStarted(Handle:hPlugin, numParams){
	...
	{
		if (isKo3Running) { return iMixmodStatus_isKo3Running; }
		else if (hasMixStarted) { return iMixmodStatus_hasMixStarted; }
	}
	return iMixmodStatus_desconocido;
}
...
{
	if (hasMixStarted && IsClientConnected(client))
	{
...
//							ct			tt
public getNombreTeam(String:str1[], String:str2[]){
	new bool:bCT = false;
	new bool:bTT = false;
	new String:sCT[50];
	new String:sTT[50];
		...
		if(IsClientInGame(i)){
			if(!bCT || !bTT){
				switch(GetClientTeam(i)){
					case CS_TEAM_CT:{
						GetClientName(i, sCT, sizeof(sCT));
						bCT = true;
					}
					case CS_TEAM_T:
					{
						GetClientName(i, sTT, sizeof(sTT));
						bTT = true;
					}
				}
			}
	...
	Format(str1, 50, "%s", sCT);
	Format(str2, 50, "%s", sTT);
}
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (hasMixStarted)
		{
			...
			Call_PushString(teamAName);
			Call_PushString(teamBName);
			switch(g_CurrentHalf){
				case 1:
					{
						Call_PushCell(g_nCTScoreH1);
						Call_PushCell(g_nTScoreH1);
					}
				case 2:
					{
						Call_PushCell(g_nTScore);
						Call_PushCell(g_nCTScore);
					}
				case 3:
					{
						Call_PushCell(g_nCTScore);
			...
			Call_PushCell(g_CurrentHalf);
			if(g_CurrentRound == 0){
				Call_PushCell(g_CurrentRound+1);
			}
			else{	
				Call_PushCell(g_CurrentRound);
			}
			Call_Finish();
			...
			else if (g_CurrentHalf == 2)
			{
				...
				else
				{
					...
					if (GetConVarInt(g_CvarShowScores) == 1)
						PrintToChatAll("\x04[%s]:\x03 Round\x03 %d \x04- Half\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamBName , g_nCTScore , teamAName, g_nTScore);
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (didLiveStarted)
		{
			if(client > 0)
				...
				PrintToServer("\x04[%s]:\x03 Live is already running!", MODNAME);
		...
		if (!hasMixStarted)
		{
			if(client > 0)
				...
				PrintToServer("\x04[%s]:\x03 Mix is not running...", MODNAME);
...
public Event_PlayerTeam(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(isKo3Running){
		if(client == g_LastWinner && new_team == CS_TEAM_SPECTATOR){
			g_LastWinner = 0;
		}
	}
...
StartAutoMix()
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		else
		{
			for (new i=1; i<=MaxClients; i++)
			{
				if (g_ReadyPlayers[i] && IsClientInGame(i))
